/* Playing with hardware alarms on the DS3234.
   ...
   set up Alarm2 to fire on the whenever time is two minutes after the hour hh:02.
   ...
   ... using a Sparkfun DeadOn RTC which is really just a DS3234 
       chip with a couple of other circuit elements on a break-out board
...
void setup() {
  ...
  SPI.transfer(B00000010); //set it at two minutes with minutes mask 
                           //bit at zero (binary number 0000 0010)
  digitalWrite(cs, HIGH);  
  delay(10); 
...
void loop() {
  ...
  // read alarm 2 flag from the DS3234 register. 
  digitalWrite(cs, LOW);
